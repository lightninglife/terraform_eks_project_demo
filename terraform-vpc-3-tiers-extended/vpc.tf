...
### Create an EKS cluster policy and attach it to the role
resource "aws_iam_policy" "eks_cluster_policy" {
  name        = "AmazonEKSClusterPolicy"
  path        = "/"
  description = "eks_cluster_policy"

  policy = file("AmazonEKSClusterPolicy.json")
}

resource "aws_iam_policy_attachment" "eks_role_attach_AmazonEKSClusterPolicy" {
  name       = "eks_role_attach_eks_cluster"
  roles      = [aws_iam_role.eks_role.name]
  policy_arn = aws_iam_policy.eks_cluster_policy.arn
}

### Create an EKS vpc resource controller policy
resource "aws_iam_policy" "eks_vpc_resource_controller_policy" {
  name        = "AmazonEKSVPCResourceControllerPolicy"
  path        = "/"
  description = "eks_vpc_resource_controller_policy"

  policy = file("AmazonEKSVPCResourceControllerPolicy.json")
}

resource "aws_iam_policy_attachment" "eks_role_attach_AmazonEKSVPCResourceControllerPolicy" {
  name       = "eks_role_attach_eks_vpc_resource_controller"
  roles      = [aws_iam_role.eks_role.name]
  policy_arn = aws_iam_policy.eks_vpc_resource_controller_policy.arn
}

### Create an EKS ec2 container registry read only policy
resource "aws_iam_policy" "eks_ec2_container_registry_ready_only_policy" {
  name        = "AmazonEC2ContainerRegistryReadOnly"
  path        = "/"
  description = "eks_ec2_container_registry_ready_only_policy"

  policy = file("AmazonEC2ContainerRegistryReadOnlyPolicy.json")
}

resource "aws_iam_policy_attachment" "eks_role_attach_AmazonEC2ContainerRegistryReadOnlyPolicy" {
  name       = "eks_ec2_container_registry_ready_only"
  roles      = [aws_iam_role.eks_role.name]
  policy_arn = aws_iam_policy.eks_ec2_container_registry_ready_only_policy.arn
}
...
resource "aws_eks_cluster" "eks_cluster" {
  ...
  vpc_config {
    subnet_ids = [aws_subnet.public_subnet_1.id, aws_subnet.subnet_2.id]
  }
  ...
  depends_on = [
    aws_iam_role_policy_attachment.eks_role_attach_AmazonEKSClusterPolicy,
    aws_iam_role_policy_attachment.eks_role_attach_AmazonEKSVPCResourceController,
    aws_iam_role_policy_attachment.eks_role_attach_AmazonEC2ContainerRegistryReadOnly,
  ]
}
...
resource "aws_eks_node_group" "eks_node_group" {
  ...
  depends_on = [
    aws_iam_role_policy_attachment.eks_role_attach_AmazonEKSWorkerNodePolicy,
    aws_iam_role_policy_attachment.eks_role_attach_AmazonEKS_CNI_Policy,
    aws_iam_role_policy_attachment.eks_role_attach_AmazonEC2ContainerRegistryReadOnly,
  ]
}
